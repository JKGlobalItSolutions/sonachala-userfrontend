...
const calculateRoomPrice = (room, adults, children, nights) => {
  const base = parseFloat(room.roomPrice || "0");
  const adultRate = parseFloat(room.perAdultPrice || "0") * adults;
  const childRate = parseFloat(room.perChildPrice || "0") * children;
  const discount = parseFloat(room.discount || "0"); // Assume this is percentage
  const totalBeforeDiscount = (base + adultRate + childRate) * nights;
  const discountedPrice = totalBeforeDiscount - (totalBeforeDiscount * (discount / 100));
...
const getTotalNights = useCallback(() => {
  ...
  bookingsSnapshot.docs.forEach((bookingDoc) => {
    ...
    const checkInRaw = bookingData["Check-In Date"];
    const checkOutRaw = bookingData["Check-Out Date"];

    if (!checkInRaw || !checkOutRaw || !checkInRaw.toDate || !checkOutRaw.toDate) {
      // Skip this booking if check-in/check-out is invalid
      return;
    }

    const bookingCheckIn = checkInRaw.toDate();
    const bookingCheckOut = checkOutRaw.toDate();
  ...
  useEffect(() => {
    ...
    // Initialize adults/children count for each room after rooms are fetched
    if (rooms.length > 0) {
      const initialAdults = {};
      const initialChildren = {};
      rooms.forEach(room => {
        initialAdults[room.id] = adultsParam;
        initialChildren[room.id] = childrenParam;
      });
    ...
    fetchRooms();
  }, [location.search, fetchRooms, rooms.length]); // Add rooms.length as dependency to run after rooms are fetched
  ...
  const incrementValue = (setter, value, roomId, isAdult) => {
    ...
    if (isAdult) {
      ...
      // Children count should not exceed adults - 1 (assuming at least one adult per child for some logic)
      // And total guests should not exceed maxGuestAllowed
      if (totalGuests < maxGuestAllowed && children[roomId] < currentAdults -1) {
        setter((prev) => ({
          ...prev,
          [roomId]: value + 1,
        }));
      } else if (adults[roomId] === 1 && totalGuests < maxGuestAllowed) {
        // Special case: If only 1 adult, allow children to be added if maxGuestAllowed permits
        setter((prev) => ({
          ...prev,
          [roomId]: value + 1,
        }));
  ...
  const decrementValue = (setter, value, roomId, isAdult) => {
    ...
    if (isAdult) {
      // Allow decrementing adults only if there's more than 1 adult
      // OR if there's 1 adult and no children (to prevent 0 adults)
      if (currentAdults > 1) {
        setter((prev) => ({
          ...prev,
          [roomId]: value - 1,
  ...
  const handleCreatePlan = useCallback(
    (room) => {
      if (availableRooms[room.id] > 0) {
        ...
        const currentAdults = adults[room.id] || 1;
        const currentChildren = children[room.id] || 0;
   const { discountedPrice } = calculateRoomPrice(room, currentAdults, currentChildren, nights);


        const newRoom = {
          ...room,
          reservationId: Date.now(), // Unique ID for each created instance
          guestCount: currentAdults + currentChildren,
          childCount: currentChildren, // This seems redundant with 'children' but kept for consistency
        ...
        toast.error("No more rooms of this type available!");
      }
    },
  ...
  const calculateOriginalTotalPrice = useCallback(() => {
    return Number.parseFloat(
      createdRoomsRef.current
        .reduce((total, room) => {
          // Re-calculate original price for created room based on its stored attributes
          ...
          <Row>
            <Col lg={6}>
              {rooms.length > 0 ? (
                ...
                <NoRoomsMessage>No room types found for this hotel.</NoRoomsMessage>
              )}
            </Col>
      ...
      {createdRooms.length > 0 && (
        <MobileCreatedRoomsPopup isOpen={showMobileCreatedRooms}>
          <ToggleButton onClick={() => setShowMobileCreatedRooms(!showMobileCreatedRooms)}>
            {showMobileCreatedRooms ? "Close Selected Rooms" : "View Selected Rooms"}
          </ToggleButton>
